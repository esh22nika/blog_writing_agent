...
# Streamlit UI
# -----------------------------
st.set_page_config(page_title="Blog Writer Agent — Ollama", layout="wide")

st.title("Blog Writing Agent")

with st.sidebar:
    st.header("⚙️ Generate New Blog")
    st.caption("Model: qwen2.5 via Ollama (local)")
    topic = st.text_area(
        "Topic",
        height=120,
    )
...
if run_btn:
    ...
    # Note: researcher uses qwen2.5 knowledge (no Tavily key needed)
...
if out:
    ...
    with tab_evidence:
        ...
        if not evidence:
            st.info("No evidence returned — running in closed_book mode (LLM-only research, no external search).")
        else:
            rows = []
            for e in evidence:
